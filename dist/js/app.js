(() => {
    ...
    const script_form = document.forms.validation;
    if (script_form) script_form.addEventListener("keyup", (function(event) {
        const inputNumIndex = getIndexFromSet(script_form, event.target);
        if (event.target.value.length >= parseInt(event.target.getAttribute("maxlength"))) if (inputNumIndex < script_form.length - 1) script_form[inputNumIndex + 1].focus(); else script_form[0].focus();
        if (0 == event.target.value.length) if (inputNumIndex > 0) script_form[inputNumIndex - 1].focus(); else script_form[script_form.length - 1].focus();
    }));
    function getIndexFromSet(set, elm) {
        let setArr = [].slice.call(set);
        for (let i in setArr) if (setArr[i] == elm) return parseInt(i);
    }
    window["FLS"] = true;
    isWebp();
    tabs();
